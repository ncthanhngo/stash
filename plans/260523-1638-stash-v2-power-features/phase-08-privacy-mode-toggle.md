...
- Create: `Stash/Application/PrivacyModeState.swift`
- Modify: `Stash/Application/HotkeyAction.swift` — add `.togglePrivacyMode`
- Modify: `Stash/Infrastructure/Hotkeys/HotkeyCenter.swift` — register hotkey
- Modify: `Stash/Infrastructure/Capture/ClipboardWatcher.swift` — early-exit on pause (inject state via closure)
- Modify: `Stash/Presentation/MenuBar/MenuBarController.swift` — observe state, swap icon
- Modify: `Stash/Application/AppDelegate.swift` — wire PrivacyModeState
- Modify: `Stash/Presentation/Settings/SettingsView.swift` — toggle row
- Create: `Stash/Resources/Assets.xcassets/MenuBarIconPaused.imageset/` (a 16×16 + 32×32 template PNG with red strike-through overlay)
