...
Chrome / Safari / Firefox WebExtension that adds "Send to Stash slot N" to the right-click menu when text is selected. Communicates with the app via macOS Native Messaging Host — a small JSON-RPC binary shipped with the app. Bypasses copy-paste entirely.

## Requirements

- **Functional:**
  - Right-click selected text in browser → "Stash → Slot 1..9 / Add to history". Selection + page URL + title sent to app, item added or pinned.
...
Browser (Chromium MV3)
   ↓ chrome.runtime.sendNativeMessage("com.soi.stash.host", payload)
NativeMessagingHost binary (stash-native-host)
   ...
   ↓
Stash app (SocketServer from Phase 1)
...
- Create: `native-host/Sources/stash-native-host/main.swift` (SwiftPM exec)
- Create: `native-host/Package.swift`
- Create: `native-host/com.soi.stash.host.json`    (NMH manifest template)
- Create: `Stash/Application/NativeHostInstaller.swift` (writes NMH manifest to per-browser locations)
- Modify: `Stash/Presentation/Settings/SettingsView.swift` — "Browser Extension" section with Install button
- Modify: `project.yml` — post-build copy NMH binary into app bundle
- Create: `StashTests/NativeHostInstallerTests.swift`
...
2. **Native Messaging Host binary** in Swift:
   ...
   - Opens Unix socket at `/tmp/stash-<uid>.sock` (Phase 1) and forwards as `IPCMessage.add(slot: ..., text: ...)` (or `.pin`).
...
3. **NMH manifest:** per-browser JSON describing the host binary path + allowed extension IDs. Locations:
   - Chrome: `~/Library/Application Support/Google/Chrome/NativeMessagingHosts/com.soi.stash.host.json`
   ...
   enum NativeHostInstaller {
       static func install(for browsers: Set<Browser>) throws {
           for browser in browsers {
               ...
               try manifest.write(to: dir.appendingPathComponent("com.soi.stash.host.json"))
           }
       }
   }
   ```
...
- [ ] In Chrome, install extension via "Load unpacked" + click "Install host manifest" in Stash settings.
- [ ] Highlight text in any page → right-click → "Stash → Slot 3" → text becomes slot 3 within 200 ms.
