...
- Modify: `Stash/Domain/TemplateTokenizer.swift` — recognise `prompt` keyword
- Modify: `Stash/Domain/TemplateRenderer.swift` — extend RenderResult, accept prompt answers dict
- Create: `Stash/Application/PromptCache.swift` — per-session memo by templateHash + label
- Create: `Stash/Presentation/Templates/PromptSheet.swift` — SwiftUI modal sheet
- Modify: `Stash/Application/AppDelegate.swift` — pasteFromSlot branches on needsPrompts
- Modify: `Stash/Application/HotstringEngine.swift` (Phase 4) — same prompt path on snippet expand
- Modify: `StashTests/TemplateRendererTests.swift` — add prompt tests
- Modify: `StashTests/TemplateTokenizerTests.swift` — add prompt token test
