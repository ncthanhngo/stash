...
- Create: `Stash/Domain/VaultItem.swift`
- Create: `Stash/Application/VaultRepository.swift`
- Create: `Stash/Application/VaultStore.swift`
- Create: `Stash/Infrastructure/Vault/KeychainVault.swift`
- Create: `Stash/Infrastructure/Vault/VaultMetadataStore.swift`  (plist persistence for titles)
- Create: `Stash/Presentation/Vault/VaultView.swift`
- Create: `Stash/Presentation/Vault/VaultEditorView.swift`
- Modify: `Stash/Presentation/Popover/ClipboardPopoverView.swift` — Vault tab
- Modify: `Stash/Application/AppDelegate.swift` — wire VaultStore
- Modify: `Stash/Application/AppDelegate.swift` — never enqueue vault content to sync
- Create: `StashTests/KeychainVaultTests.swift` (uses test service ID)
...
2. **`KeychainVault`** wraps Security.framework:
   ...
   final class KeychainVault {
       private let service = "com.soi.stash.vault"
...
3. **`VaultMetadataStore`** persists `[VaultItem]` (no secrets) as plist at `~/Library/Application Support/Stash/vault.plist`. CRUD operations are fast (no Keychain access).
...
- [ ] Vault items do NOT appear in sync folder (`Stash/slot-*.json` never written for vault).
