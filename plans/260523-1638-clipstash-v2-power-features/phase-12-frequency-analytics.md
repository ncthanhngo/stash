...
- **Functional:**
  ...
  - Proactive banner shown once per item per week if paste_count crosses threshold (default 5/week) and item not pinned. Suppressed via `clipstash.insightsSuppressed.<itemHash>` UserDefaults key.
...
- Modify: `Clipstash/Infrastructure/Storage/Migrations.swift` — v4 migration
- Modify: `Clipstash/Infrastructure/Storage/ClipboardRecord.swift` — fields
- Modify: `Clipstash/Application/ClipboardRepository.swift` — `recordPaste(itemID:)` protocol method
- Modify: `Clipstash/Infrastructure/Storage/GRDBClipboardRepository.swift` — implement
- Modify: `Clipstash/Infrastructure/Paste/SystemPasteEngine.swift` — accept post-paste callback
- Modify: `Clipstash/Application/ClipboardStore.swift` — call recordPaste after paste
- Create: `Clipstash/Application/InsightsService.swift` — compute top-10 + banner candidates
- Create: `Clipstash/Presentation/Insights/InsightsView.swift`
- Create: `Clipstash/Presentation/Insights/PinPromptBanner.swift`
- Modify: `Clipstash/Presentation/Settings/SettingsView.swift` — Insights tab
- Modify: `Clipstash/Presentation/Popover/ClipboardPopoverView.swift` — show banner if present
- Create: `ClipstashTests/InsightsServiceTests.swift`
...
4. **`PinPromptBanner`** SwiftUI view: orange-tinted card at top of popover showing item preview + "Pin to slot N" (suggests lowest empty slot) + "Dismiss" + "Don't show again". On "Don't show again", set `UserDefaults.standard.set(true, forKey: "clipstash.insightsSuppressed.\(item.contentHash)")`.
