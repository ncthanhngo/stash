...
- **Functional:** Works on any captured image (text/code screenshots, photos of receipts, signs). Auto-detects language. Supports multi-line. Inserts result as new text history item with source `Clipstash · OCR`.
...
- Create: `Clipstash/Infrastructure/OCR/OCRService.swift`
- Create: `Clipstash/Infrastructure/OCR/OCRError.swift`
- Modify: `Clipstash/Application/ClipboardStore.swift` — add `extractText(from item:)`
- Modify: `Clipstash/Presentation/Popover/ClipboardPopoverView.swift` — context menu entry for image items
- Create: `ClipstashTests/OCRServiceTests.swift` (uses a small bundled fixture image)
- Create: `ClipstashTests/Fixtures/ocr-sample.png` (small image with known text)
...
6. **Test fixture:** add a small `ocr-sample.png` (e.g., a 200×60 image of the text "Hello World") to `ClipstashTests/Fixtures/`. Test asserts `recognize(...)` returns "Hello World" (allow ±2-char Levenshtein for Vision flakiness).
