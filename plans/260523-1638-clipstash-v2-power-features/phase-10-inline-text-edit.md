...
- Modify: `Clipstash/Application/ClipboardStore.swift` — add editingItemID + commitEdit/cancelEdit
- Modify: `Clipstash/Presentation/Popover/HistoryRow.swift` — switch view based on editing state
- Modify: `Clipstash/Presentation/Popover/ClipboardPopoverView.swift` — handle double-tap gesture
- Create: `ClipstashTests/InlineEditTests.swift`  (commit creates new item, original unchanged)
...
1. **Store state:**
   ...
   func commitEdit() {
       ...
       let item = ClipboardItem(content: content, contentHash: ContentHasher.hash(content), sourceAppName: "Clipstash · edit")
