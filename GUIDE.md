...
| Tính năng | Mô tả |
|-----------|-------|
| **URL scheme** | `stash://paste/3` · `stash://add?text=hi&slot=5` · `stash://open` — gọi từ Raycast, Alfred, shell script, browser. |
| **CLI wrapper** | `scripts/stash paste 3` · `scripts/stash add "text" --slot 7` — wrapper gọi URL scheme. |
| **Browser extension** | Chrome/Brave/Edge MV3 extension: right-click selected text → Send to Stash → Add to history hoặc Pin to slot 1-9. |
...
cp -R build/Build/Products/Release/Stash.app ~/Applications/
...
1. macOS system: "Stash" wants to control your computer → **Open System Settings**
2. App alert: "Stash needs Accessibility permission" → **Open Accessibility Settings**

Trong **System Settings → Privacy & Security → Accessibility**:
- Tìm **Stash** trong list
- Toggle **ON**
- Quit + reopen Stash (`pkill -x Stash && open ~/Applications/Stash.app`)
...
**Cảnh báo:** `⇧⌘S` cũng là "Save As" trong nhiều app (Chrome, Safari, Notes…). Hotkey global của Stash sẽ intercept trước — Save As của app khác không kích hoạt được khi Stash đang chạy. Backup dùng `⇧⌘5` (toolbar screenshot của macOS).
...
Chọn 1 folder đã được OneDrive/iCloud Drive/Dropbox/Google Drive đồng bộ sẵn. App tạo subfolder `Stash/` trong đó, ghi per-slot JSON + PNG files.
...
# Add text to history
open "stash://add?text=Hello%20world"

# Pin text to slot 5
open "stash://add?text=API_KEY%3Dxxx&slot=5"
...
```bash
sudo ln -s ~/dev/06-memory/scripts/stash /usr/local/bin/stash

stash paste 3
stash add "Hello"
stash add "Secret" --slot 7
stash open
```
...
2. Right-click → **Send to Stash** → submenu:
   - **Add to history**
   - **Pin to slot 1-9**
3. **Lần đầu** Chrome hỏi "Open Stash?" → tick **Always allow** + Open
4. Sau đó silent — text vào Stash trong < 200ms

**Privacy**: extension chỉ chạy khi bạn right-click có selection. Không đọc trang. Không gửi network. Chỉ trigger URL scheme `stash://`.
...
2. System Settings → Privacy & Security → Accessibility → bật **Stash**
3. **Quit + reopen** Stash (process cache trust ở startup)
...
- Nếu vẫn trống: kiểm tra log `log show --predicate 'subsystem == "com.soi.stash"' --last 1m`
- Reset DB: `rm ~/Library/Application\ Support/Stash/stash.sqlite*` rồi relaunch (mất hết history!)

### URL scheme `stash://` không trigger

```bash
# Re-register app với LaunchServices
/System/Library/Frameworks/CoreServices.framework/Versions/A/Frameworks/LaunchServices.framework/Versions/A/Support/lsregister -f -R ~/Applications/Stash.app
```
...
Hoặc:
```bash
xcodebuild -project Stash.xcodeproj -scheme Stash -configuration Debug \
    -derivedDataPath /tmp/stash-test \
...
**Fix lâu dài**: dùng signing identity "Stash Dev" (script `scripts/create-dev-cert.sh` đã setup). Identity ổn định qua nhiều build → permission persist.
...
- **Privacy guarantee**: code-level — không có `URLSession`, không có analytics framework. Grep `~/dev/06-memory/Stash/` để verify.
