...
/// Parses incoming `clipstash://` URLs and dispatches to app actions.
@MainActor
final class URLSchemeHandler {
    private static let log = Logger(subsystem: "com.soi.clipstash", category: "url-scheme")
    ...
    nonisolated static func parse(_ url: URL) -> Command? {
        guard url.scheme == "clipstash" else { return nil }
