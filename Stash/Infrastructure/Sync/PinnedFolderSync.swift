...
final class PinnedFolderSync: ObservableObject {
    ...
    private static let folderKey = "stash.sync.folder"
    private static let deviceKey = "stash.device.id"
    ...
    func enable(folderURL parentURL: URL) {
        disable()
        let subfolder = parentURL.appendingPathComponent("Stash", isDirectory: true)
        do {
